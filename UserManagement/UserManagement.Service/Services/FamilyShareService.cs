...
{
    private readonly IFamilyShareRepository _familyShareRepository;
    private readonly IUserRepository _userRepository;

    public FamilyShareService(IFamilyShareRepository familyShareRepository, IUserRepository userRepository)
    {
        _familyShareRepository = familyShareRepository ?? throw new ArgumentNullException(nameof(familyShareRepository));
    ...
    public async Task<FamilyShareResponseDto> CreateAsync(CreateFamilyShareDto dto, CancellationToken cancellationToken = default)
    {
        ...
        var existing = await _familyShareRepository.GetByOwnerAndMemberAsync(dto.OwnerUserId, dto.MemberUserId, cancellationToken);
        if (existing != null)
            throw new FamilyShareAlreadyExistsException(dto.OwnerUserId, dto.MemberUserId);

        var owner = await _userRepository.GetByIdAsync(dto.OwnerUserId, cancellationToken);
        if (owner == null)
        ...
        var member = await _userRepository.GetByIdAsync(dto.MemberUserId, cancellationToken);
        if (member == null)
        ...
        var saved = await _familyShareRepository.AddAsync(familyShare, cancellationToken);
        return MapToResponse(saved);
    }

    public async Task<FamilyShareResponseDto?> GetByIdAsync(Guid shareId, CancellationToken cancellationToken = default)
    {
        var share = await _familyShareRepository.GetByIdAsync(shareId, cancellationToken);
    ...
    {
        var shares = await _familyShareRepository.GetByOwnerIdAsync(ownerUserId, cancellationToken);
    ...
    {
        var shares = await _familyShareRepository.GetByMemberIdAsync(memberUserId, cancellationToken);
        return shares.Select(MapToResponse);
    }
    ...
    public async Task<FamilyShareResponseDto> UpdateAsync(Guid shareId, UpdateFamilyShareDto dto, CancellationToken cancellationToken = default)
    {
        ...
        var updated = await _familyShareRepository.UpdateAsync(share, cancellationToken);
        return MapToResponse(updated);
    }

    public async Task<bool> DeleteAsync(Guid shareId, CancellationToken cancellationToken = default)
    {
        return await _familyShareRepository.DeleteAsync(shareId, cancellationToken);
    }
